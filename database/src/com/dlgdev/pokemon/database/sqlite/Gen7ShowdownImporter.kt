...
class Gen7ShowdownImporter @Inject constructor(val dao: PokemonDao) : ShowdownImporter {
    ...
    private fun abilities(abilities: JSONObject): Array<com.dlgdev.pokemon.database.Ability?> {
        val output: Array<com.dlgdev.pokemon.database.Ability?> = arrayOfNulls(3)
        output[0] = com.dlgdev.pokemon.database.Ability(abilities.getString("0"))
        output[1] = if (abilities.has("1")) com.dlgdev.pokemon.database.Ability(abilities.getString("1")) else null
        output[2] = if (abilities.has("H")) com.dlgdev.pokemon.database.Ability(abilities.getString("H")) else null
        return output
    }
